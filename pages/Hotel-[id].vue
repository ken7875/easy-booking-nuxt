<template>
  <div>
    <Suspense>
                ...
                <Card>
                  ...
                  <template v-slot:body>
                    <div class="border-b border-darkLight p-[1rem]">
                      ...
                      <span class="bg-[#28A745] rounded-[8px] p-2 mr-2 text-white">{{
                        hotelDetailData?.ratingAverage
                      }}</span>
                      <span>超級讚</span>
                    </div>
                ...
                <div class="flex items-center justify-between">
                  ...
                  <div class="mx-2 flex items-center">
                    ...
                    <Button
                      ...
                      @click="reserveHotelInfo.bookingNum++"
                      :disabled="
                        reserveHotelInfo.roomTypeInfo?.roomNum &&
                        reserveHotelInfo.roomTypeInfo?.roomNum <= reserveHotelInfo.bookingNum
                      "
                    >
                      +
                    </Button>
      ...
      <template #fallback>
        <Loading class="fixed top-[50%] left-[50%] translate-[-50%]" />
      </template>
    </Suspense>
  </div>
</template>
...
const hotelDetailData = ref<Hotel>();

// const getHotelDetailData = async () => {
//   const { data } = await useAsyncData('hotelDetail', () => getProductApi<Hotel>(hotelId.value));

//   hotelDetailData.value = data.value?.data.data;
